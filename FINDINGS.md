...
**Alpha Leak Audit (2026-07-13):** dominant leak = ETF-validated book traded through CFD cost stack (financing+spread+commission) + live vix/DD throttle absent from backtest -> est. 25-40% Sharpe haircut before any strategy is 'wrong'. S3 -73% frequency (rule drift). Parity fixes only, post-window. docs/ALPHA_LEAK_AUDIT.md
**Forensic Investigation (2026-07-13):** gates NOT over-rejecting (S5 breakout gate does 91% of filtering; regime gates reject 485/7448); low live freq = downtime+venue+S3-drift, not filters. S5 strongly risk-on-conditional but mostly explanatory (only VIX-ts shadow-validated). Highest-EV action = re-validate with real cost stack, not new alpha. docs/FORENSIC_INVESTIGATION.md
**BUG FIX (2026-07-13): overnight CRASH** `TypeError: unsupported format string passed to NoneType.__format__`. Root cause broker.py place_order_safe line 109/201: guard checked `sl is not None` then formatted BOTH sl AND tp with :.2f; OVN enters sl-only (time-exit, no target) -> `TP={None:.2f}` crashed. Unique to session=overnight (only sl-only caller). Log-only fix: format each leg independently. No strategy/signal/sizing/exit change.
**Forensic Pipeline Audit (2026-07-14):** full research→evidence trace; signal SHAPE faithful, loss concentrated at Validation→Execution (ETF-costed validation + throttle vs CFD live). No unexplained missing trades. Phase-4 live R/latency/rejects = INSUFFICIENT DATA (need VPS fills.csv + MT5 history). Ranked fixes: re-cost validation, merge VPS evidence, ETF routing, OVN fix — all no-clock-reset — before any signal change. docs/FORENSIC_PIPELINE_AUDIT.md
**Operational Readiness Audit (2026-07-14):** #1 daily kill-switch effectively DISABLED (session_start_equity=0 default -> never fires) = top risk; #2 no MT5 mid-session reconnect (drop crashes session); #3 alerts fail silently; #4 single-VPS no external watchdog. Ranked P*I checklist, Tier 0 before real money. Audit-only, no code change. docs/OPERATIONAL_READINESS_AUDIT.md
**Research→Live Forensics (2026-07-14):** strategies FUNDAMENTALLY SOUND; gates NOT too restrictive (S1 sweep rejects 87%, S5 breakout 66% = the mechanism, not over-filtering; no gate destroys expectancy). No strategy lost validity; S3 live rule is a validated subset (97% overlap), not invalid. Gap = CFD cost + risk overlay + downtime. Live R/latency Unknown (need VPS fills). docs/RESEARCH_TO_LIVE_FORENSICS.md
**Live Evidence Reconciliation (2026-07-14):** VPS inputs (fills.csv/MT5 history/mt5 logs) MISSING on this host -> live reconciliation INSUFFICIENT_DATA (Mac logs NOT substituted; exact export steps given). MEASURED re-cost A/B/C/D on S5: ETF faithful (Sharpe 1.40=1.40), CFD financing dominant leak (1.40->0.89), throttle neutral (0.93). Status: S3 REVALIDATE, OVN INVESTIGATE_OPERATIONAL, rest INSUFFICIENT_DATA. docs/LIVE_EVIDENCE_RECONCILIATION.md
